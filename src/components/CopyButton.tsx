...
const CopyButton = ({ code }: { code: string }) => {
  ...
  const copyCode = () => {
    if (navigator?.clipboard) {
      const matcher = (x: any) =>
        typeof x === "string" && x.includes("<token>");
      const masker = (x: any) => x.replace(/<token>/g, project?.api_key);
      const replacer = project && matcher(code) ? masker : (x: any) => x;

      navigator.clipboard.writeText(replacer(code));
    }
  };
